...
def gigform():
    ...
    if request.method == "GET" and session.get('user_id', None) is not None:
        ...
        if x is None:
            users.insert({'email':email,'gigs':[],'MDcontents':[],'youtube':[],'avatar':avatar,'location':location,'name':giturl.split('https://github.com/')[1],'thumbsup':[]})
    ...
    if request.method == "POST":
        ...
        if gig !="" and user != "":
            if gig in gigs:
                ...
                users.update({'name':user},{'$push':{'thumbsup':likes}})
